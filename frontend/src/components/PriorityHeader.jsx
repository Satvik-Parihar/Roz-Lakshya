  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[color:var(--outline-variant)]/60 bg-[color:var(--surface-container)]/95 shadow-sm backdrop-blur">
      <div className="grid w-full grid-cols-[auto_1fr_auto] items-center gap-2 px-3 py-3 md:grid-cols-[1fr_auto_1fr] md:gap-3 md:px-5 lg:gap-4 lg:px-8">
        <Link 
          to={authSnapshot.mustResetPassword ? '#' : (appMode ? '/dashboard' : '/')} 
          className={`flex min-w-0 items-center gap-2 ${authSnapshot.mustResetPassword ? 'cursor-default pointer-events-none' : ''}`} 
        ...
        <nav className="hidden items-center justify-center gap-2 md:flex">
          {appMode && !authSnapshot.mustResetPassword ? (
            appLinks.map((item) => (
              <NavLink key={item.to} to={item.to} className={navClass}>
                {item.label}
              </NavLink>
            ))
          ) : !authSnapshot.mustResetPassword ? (
            <>
        ...
        <div className="hidden items-center justify-self-end gap-3 md:flex">
          {authSnapshot.isAuthenticated ? (
            <div className="relative" ref={profileRef}>
              <button
                type="button"
                onClick={() => !authSnapshot.mustResetPassword && setProfileOpen((v) => !v)}
                className={`inline-flex h-9 items-center justify-center gap-2 rounded-full border px-3 text-xs font-semibold transition-colors ${
                  authSnapshot.mustResetPassword 
                    ? 'cursor-not-allowed border-[color:var(--outline-variant)] bg-[color:var(--surface-container-low)] text-[color:var(--on-surface-variant)]'
                    : profileOpen
              ...
              {profileOpen && !authSnapshot.mustResetPassword && (
